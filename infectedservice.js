...
exports.getVirusV2 = function *(userid) {
    ...
    console.log(random);
    var order = yield mongodb.collection('order').aggregate([
        {$match:{
            "vid":{$nin:ifs}
        }},
        {
    ...
    order = order[0];
    if(!order){
        order = yield mongodb.collection('order').findOne({$and: [{vid:{$ne:null}},{vid: {$nin: ifs}},
            {$or: [{$and: [{speed: true}, {fullfill: {$lt: 16}}]},
                {$and: [{speed: false}, {fullfill: {$lt: 4}}]}
            ]},{$or:[{rnd:{$gte:random}},{rnd:{$lte:random}}]}]}
        );}
    if (!order){
        var data = {'head':{code: 1000,msg:'no virus'}};
        return data
       ...
       /* yield mongodb.collection('order').updateOne({'orderid':order.orderid},{$inc:{'fullfill':1}});
        yield mongodb.collection('infected').insertOne({'carryid':order.userid,'vid':order.vid,'infectid':userid,'orderid':order.orderid,'createtime':Date.parse(new Date()),'createdate':new Date()});*/
        ...
        var patients = yield mongodb.collection('infected').count({"vid":order.vid});

        var favor = yield mongodb.collection('action').count({"vid":order.vid,"action":"spread"})

        var speed = yield mongodb.collection('order').count({"vid":order.vid,"speed":true});
        //var favor = yield mongodb.collection('action').find({'vid':order.vid,'action':'spread'}).toArray();
        var data ={};
        data.virus = virus;
        data.userinfo = userinfo;
        data.virus.scanCount = patients;
        data.patientNumber = patients;
        data.virus.favorCount = favor;
        data.favorCount = favor;
        data.virus.speedCount = speed;
        return {'head':{code:200,msg:'success'},'data':data};
    }
    /*var order = yield mongodb.collection('order').findOne({'orderid':orderid[0].orderid})
